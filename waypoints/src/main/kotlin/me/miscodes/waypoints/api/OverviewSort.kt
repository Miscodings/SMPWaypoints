...
enum class OverviewSort(private val comparator: Comparator<GUIDisplayable>) :
  ...
  /**
   * This sorts first by [me.miscodes.waypoints.api.gui.GUIType] and then by name if the type
   * matches, both ascending
   */
  TYPE_ASCENDING(compareBy<GUIDisplayable> { it.guiType }.thenBy { it.name }),

  /**
   * This sorts first by [me.miscodes.waypoints.api.gui.GUIType] (descending) and then by name if
