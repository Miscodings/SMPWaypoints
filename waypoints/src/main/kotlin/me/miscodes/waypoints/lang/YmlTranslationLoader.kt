...
class YmlTranslationLoader(
  ...
  private lateinit var booleanSettings: Map<String, Boolean>
  ...
  private fun processConfiguration(languageConfig: FileConfiguration) {
    ...
    languageConfig.getKeys(true).forEach {
      when {
        languageConfig.isString(it) ->
            stringMap[it] = languageConfig.getString(it)!!.trimEnd(isNewLine)
        languageConfig.isBoolean(it) -> booleanMap[it] = languageConfig.getBoolean(it)
      }
    }

    translations = stringMap
    booleanSettings = booleanMap
  }
  ...
  fun loadLanguage(languageKey: String) {
    ...
    if (languageFile.exists()) {
      ...
      processConfiguration(loadedTranslations)
    } else {
