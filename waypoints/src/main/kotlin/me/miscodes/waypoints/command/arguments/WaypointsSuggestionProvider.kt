package me.miscodes.waypoints.command.arguments
    ...
    return plugin.future {
      ...
      if (allowGlobals) {
        ...
        plugin.api.publicWaypoints.searchWaypoints(query.removePrefix(publicPrefix)).forEach {
          ...
          builder.suggest(
              formatSuggestion("$publicPrefix${it.indexedName}"),
              it.t.getTooltip(sender),
          )
        }
        plugin.api.permissionWaypoints
            ...
            .forEach {
              ...
              builder.suggest(
                  formatSuggestion("$permissionPrefix${it.indexedName}"),
                  it.t.getTooltip(sender),
              )
            }
      }
