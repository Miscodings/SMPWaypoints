...
import me.miscodes.waypoints.config.WorldNotFoundAction
  ...
  override suspend fun getContent(): PaginationList<GUIDisplayable> {
    ...
    if (wpGUI.isOwner && guiFolder === wpGUI.targetData) {
      if (wpGUI.viewerData.showGlobals) {
        if (wpGUI.plugin.waypointsConfig.features.publicWaypoints) {
          val public = wpGUI.plugin.api.publicWaypoints
          if (
              public.getWaypointsAmount() > 0 ||
                  wpGUI.viewer.hasPermission(WaypointsPermissions.MODIFY_PUBLIC)
          ) {
            content.add(public)
          }
        }
        if (wpGUI.plugin.waypointsConfig.features.permissionWaypoints) {
          val permission = wpGUI.plugin.api.permissionWaypoints
          if (
              permission.getWaypointsVisibleForPlayer(wpGUI.viewer) > 0 ||
      ...
      if (
          wpGUI.plugin.waypointsConfig.playerTracking.enabled &&
    ...
    if (
        guiFolder.type == Type.PERMISSION &&
            !wpGUI.viewer.hasPermission(WaypointsPermissions.MODIFY_PERMISSION)
    ) {
        ...
        if (it is Waypoint) {
          if (it.location.world === null) {
            ...
            itr.remove()
              ...
              wpGUI.plugin.waypointsConfig.general.hideWaypointsFromDifferentWorlds &&
